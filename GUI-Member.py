...
#     T = ttk.Label(GUI,text=text,font=(None,15)).pack()
#     E = ttk.Entry(GUI,textvariable= strvar,font=font)
#     return E

# def ET2(GUI,text,strvar,font=('Angsana new',20)):
#     T = ttk.Label(GUI,text=text,font=(None,15)) # NO PACK
#     E = ttk.Entry(GUI,textvariable= strvar,font=font)
#     return (E,T)
...
E44.pack()
v_point.set('0') # ใส่ค่า deflault ของ point

# E43.bind('<Return>', lambda x: print(v_usertype.get())) # Check test
...
def DeleteMember(event):
    selete = table_member.selection() # เลือก item
    data = table_member.item(selete)['values']
    print(data)
...
def UpdateMemberInfo(event):
    selete = table_member.selection() # เลือก item
    code = table_member.item(selete)['values'][0]
